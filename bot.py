...
TOKEN = "8867735730:AAF7rZ4PvvqpP5IQXbfzBKOaW2nqc0MOtUI"
...
def get_body_fat_friend(message):
    ...
    try:
        ...
        show_result(message, "friend", "📊 **НОРМА ДЛЯ БЛИЗКОГО**")
    except:
        bot.send_message(user_id, "❌ Введи число! Попробуй еще раз.")
        bot.register_next_step_handler(message, get_body_fat_friend)
...
def show_result(message, person, title):
    ...
    if person == "self":
        ...
        # Отдельное сообщение с пояснением к кнопке
        bot.send_message(
            user_id,
    ...
    # ===== ОБЩАЯ ЧАСТЬ (канал и /reset) =====
    final_message = (
        f"————————————————\n\n"
        f"🔥 Если хочешь получать ещё больше полезных советов и рецептов — подпишись на мой канал:\n"
        f"👉 [ФитБаза]({CHANNEL_LINK})\n\n"
        f"————————————————\n\n"
...
def handle_friend_request(message):
    ...
    msg = bot.send_message(
        user_id,
        "👋 Отлично! Теперь ответь на вопросы для своего близкого.\n\n"
...
while True:
    ...
    try:
        ...
        print(">>> infinity_polling() завершился!")
