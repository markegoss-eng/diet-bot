...
if not TOKEN:
    raise ValueError("❌ BOT_TOKEN не задан! Добавьте переменную окружения.")
...
def set_webhook():
    ...
    if not webhook_url:
        print("❌ WEBHOOK_URL не задан! Проверь переменные окружения.")
        return
    ...
    # Устанавливаем новый
    webhook = bot.set_webhook(url=webhook_url)
    if webhook:
